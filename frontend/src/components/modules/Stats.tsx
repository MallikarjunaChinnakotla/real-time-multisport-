...
export default function Stats() {
  ...
  if (activeMatch && stats.scores) {
      ...
      if (sport === 'cricket') {
          ...
          let t1OverWickets: Record<number, number> = {};
          let t2OverWickets: Record<number, number> = {};
          ...
          matchScores.forEach((s: any) => {
              ...
              if (s.wicket === 'Yes' || s.wicket === '1') {
                   ...
                   if (isT1) {
                       t1OverWickets[theOver] = (t1OverWickets[theOver] || 0) + 1;
                   }
                   if (isT2) {
                       t2OverWickets[theOver] = (t2OverWickets[theOver] || 0) + 1;
                   }
              }
          ...
          for (let i = 0; i <= maxOver; i++) {
              ...
              cricketPerOverData.push({
                  ...
                  t1Wickets: t1OverWickets[i] || 0,
                  t2Wickets: t2OverWickets[i] || 0
              });
          }
          ...
          if (sport === 'kabaddi') {
              ...
              matchScores.forEach((s: any) => {
                  ...
                  const eType = (s.event_type || '').toLowerCase();
                  if (eType === 'raid' || eType === 'bonus') {
                       totalRaids++;
                       if (pts > 0) successfulRaids++;
                  }
                  if (eType === 'empty_raid' || eType === 'empty') {
                       totalRaids++;
                       emptyRaids++;
                  }
                  if (eType === 'tackle' || eType === 'super_tackle' || eType === 'super tackle') {
                       totalTackles++;
          ...
          if (sport === 'kabaddi' || sport === 'basketball' || sport === 'table_tennis') {
              ...
              if (sport === 'kabaddi') {
                  const periods = ['0-10 m', '10-20 m', '20-30 m', '30-40 m'];
                  let pScores = [ {t1:0, t2:0}, {t1:0, t2:0}, {t1:0, t2:0}, {t1:0, t2:0} ];
                  let maxP = -1;
                  matchScores.forEach((s: any) => {
                      let m = parseInt(s.minute) || 0;
                      let pIdx = Math.floor(m / 10);
                      if(pIdx > 3) pIdx = 3;
                      maxP = Math.max(maxP, pIdx);
                      const isT1 = s.team_id == t1Id || s.batting_team == t1Id;
                      const eType = (s.event_type || '').toLowerCase();
                      let pts = 0;
                      if (eType !== 'out' && eType !== 'timeout' && !eType.includes('card') && eType !== 'empty_raid' && eType !== 'empty' && eType !== 'two_min_suspension' && eType !== 'foul') {
                          pts = parseInt(s.points) || 1;
                      }
                      if (isT1) pScores[pIdx].t1 += pts;
                      else pScores[pIdx].t2 += pts;
                  });
                  for (let i = 0; i <= maxP; i++) {
                      quarterData.push({ name: periods[i], [t1Name]: pScores[i].t1, [t2Name]: pScores[i].t2 });
                  }
              } else {
                  for (let i = 0; i < numQuarters; i++) {
                      ...
                      let t1QScore = 0, t2QScore = 0;
                      chunk.forEach((c: any) => { t1QScore += c.t1Inc; t2QScore += c.t2Inc; });
                      quarterData.push({ name: `${chunkLabel} ${i+1}`, [t1Name]: t1QScore, [t2Name]: t2QScore });
                  }
              }
          }

          // Generic Player Stats
          let pTotals: Record<string, any> = {};
          matchScores.forEach((s: any) => {
              if (s.player_id && s.player_id.toString().trim() !== '') {
                  let eType = (s.event_type||'').toLowerCase();
                  let pts = 0;
                  if (eType !== 'out' && eType !== 'timeout' && !eType.includes('card') && eType !== 'empty_raid' && eType !== 'empty' && eType !== 'two_min_suspension' && eType !== 'foul') {
                      pts = parseInt(s.points) || 1;
                  }
                  
                  if (!pTotals[s.player_id]) {
                       const pInfo = stats.players?.find((p:any) => p.player_id == s.player_id || p.player_name == s.player_id);
                       const name = pInfo ? pInfo.player_name : s.player_id;
                       pTotals[s.player_id] = { name, team: getTeamName(s.team_id || s.batting_team), points: 0, actions: 0 };
                  }
                  
                  if (pts > 0) {
                      pTotals[s.player_id].points += pts;
                      pTotals[s.player_id].actions += 1;
                  }
              }
          });
          genericPlayerStats = Object.values(pTotals).filter((p:any) => p.points > 0).sort((a:any, b:any) => b.points - a.points);
      }
  }
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        {mid && chartData.length > 0 ? (
            <div className="mt-8 pt-6 border-t border-gray-100">
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={cricketPerOverData} margin={{ top: 20, right: 30, left: 10, bottom: 10 }}>
                                        ...
                                        <Bar dataKey={getTeamName(activeMatch.team1_id)} fill="#3b82f6" radius={[4, 4, 0, 0]} barSize={20}>
                                            <LabelList dataKey="t1Wickets" content={({x, y, width, value}: any) => {
                                                if (!value) return null;
                                                return <circle cx={x + width / 2} cy={y - 8} r={6} fill="#ef4444" />;
                                            }} />
                                        </Bar>
                                        <Bar dataKey={getTeamName(activeMatch.team2_id)} fill="#f97316" radius={[4, 4, 0, 0]} barSize={20}>
                                            <LabelList dataKey="t2Wickets" content={({x, y, width, value}: any) => {
                                                if (!value) return null;
                                                return <circle cx={x + width / 2} cy={y - 8} r={6} fill="#ef4444" />;
                                            }} />
                                        </Bar>
                ...
                {/* Generic Player Leaderboard (Non-Cricket) */}
                {sport !== 'cricket' && genericPlayerStats.length > 0 && (
                    <div className="mb-6">
                       <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">🏅 Player Impact Leaderboard</h3>
                           ...
                           <table className="w-full text-sm text-left">
                               <thead className="bg-gray-50 text-gray-600 border-b border-gray-200">
                                   <tr>
                                       ...
                                       <th className="px-4 py-3 text-center">Scoring Actions</th>
                                       <th className="px-4 py-3 text-center">Total Points Generated</th>
                                   </tr>
                               </thead>
                               <tbody className="divide-y divide-gray-100">
                                   {genericPlayerStats.map((p: any, i: number) => (
                                       <tr key={i} className={`hover:bg-gray-50 ${i < 3 ? 'bg-yellow-50/30 font-medium' : ''}`}>
                                           <td className="px-4 py-2 text-gray-800 flex items-center gap-2">
                                              {i === 0 && '🥇'} {i === 1 && '🥈'} {i === 2 && '🥉'}
                                              {p.name}
                                           </td>
                                           <td className="px-4 py-2 text-gray-600 font-medium">{p.team}</td>
                                           <td className="px-4 py-2 text-gray-700 text-center">{p.actions}</td>
                                           <td className="px-4 py-2 text-blue-700 font-bold text-lg text-center">{p.points}</td>
                                       </tr>
                                   ))}
                               </tbody>
                           </table>
